...
/// The `ChainSpec parametrised for moonbeam runtime`.
pub type MoonbeamChainSpec = service::GenericChainSpec<
	moonbeam_runtime::GenesisConfig,
	Extensions,
>;
...
pub fn moonbeam_config() -> Result<MoonbeamChainSpec, String> {
	MoonbeamChainSpec::from_json_bytes(&include_bytes!("../res/moonbeam.json")[..])
}
...
fn moonbeam_session_keys(
	babe: BabeId,
	grandpa: GrandpaId,
	im_online: ImOnlineId,
	parachain_validator: ValidatorId,
	authority_discovery: AuthorityDiscoveryId
) -> moonbeam_runtime::SessionKeys {
	moonbeam_runtime::SessionKeys { babe, grandpa, im_online, parachain_validator, authority_discovery }
}
...
fn moonbeam_staging_testnet_config_genesis() -> moonbeam_runtime::GenesisConfig {
	// subkey inspect "$SECRET"
	let endowed_accounts = vec![
		// 5FeyRQmjtdHoPH56ASFW76AJEP1yaQC1K9aEMvJTF9nzt9S9
		hex!["9ed7705e3c7da027ba0583a22a3212042f7e715d3c168ba14f1424e2bc111d00"].into(),
	...
	moonbeam_runtime::GenesisConfig {
		system: Some(moonbeam_runtime::SystemConfig {
			code: moonbeam_runtime::WASM_BINARY.to_vec(),
			changes_trie_config: Default::default(),
		}),
		balances: Some(moonbeam_runtime::BalancesConfig {
			balances: endowed_accounts.iter()
				.map(|k: &AccountId| (k.clone(), ENDOWMENT))
				.chain(initial_authorities.iter().map(|x| (x.0.clone(), STASH)))
				.collect(),
		}),
		indices: Some(moonbeam_runtime::IndicesConfig {
			indices: vec![],
		}),
		session: Some(moonbeam_runtime::SessionConfig {
			keys: initial_authorities.iter().map(|x| (
				x.0.clone(),
				x.0.clone(),
				moonbeam_session_keys(x.2.clone(), x.3.clone(), x.4.clone(), x.5.clone(), x.6.clone()),
			)).collect::<Vec<_>>(),
		}),
		babe: Some(Default::default()),
		grandpa: Some(Default::default()),
		im_online: Some(Default::default()),
		authority_discovery: Some(moonbeam_runtime::AuthorityDiscoveryConfig {
			keys: vec![],
		}),
		parachains: Some(moonbeam_runtime::ParachainsConfig {
			authorities: vec![],
		}),
		registrar: Some(moonbeam_runtime::RegistrarConfig {
			parachains: vec![],
			_phdata: Default::default(),
		}),
		vesting: Some(moonbeam_runtime::VestingConfig {
			vesting: vec![],
		}),
		sudo: Some(moonbeam_runtime::SudoConfig {
			key: endowed_accounts[0].clone(),
		}),
	}
}
...
/// Moonbeam staging testnet config.
pub fn moonbeam_staging_testnet_config() -> MoonbeamChainSpec {
	MoonbeamChainSpec::from_genesis(
		"Moonbeam Staging Testnet",
		"moonbeam_staging_testnet",
		ChainType::Live,
		moonbeam_staging_testnet_config_genesis,
		Vec::new(),
		Some(TelemetryEndpoints::new(vec![(WESTEND_STAGING_TELEMETRY_URL.to_string(), 0)])
			.expect("Moonbeam Staging telemetry url is valid; qed")),
		Some(DEFAULT_PROTOCOL_ID),
		None,
		Default::default(),
	)
...
/// Helper function to create moonbeam GenesisConfig for testing
pub fn moonbeam_testnet_genesis(
	initial_authorities: Vec<(AccountId, AccountId, BabeId, GrandpaId, ImOnlineId, ValidatorId, AuthorityDiscoveryId)>,
	root_key: AccountId,
	endowed_accounts: Option<Vec<AccountId>>,
) -> moonbeam_runtime::GenesisConfig {
...
fn moonbeam_local_testnet_genesis() -> moonbeam_runtime::GenesisConfig {
	moonbeam_testnet_genesis(
		vec![
			get_authority_keys_from_seed("Alice"),
			get_authority_keys_from_seed("Bob"),
		],
...
/// Moonbeam local testnet config (multivalidator Alice + Bob)
pub fn moonbeam_local_testnet_config() -> MoonbeamChainSpec {
	MoonbeamChainSpec::from_genesis(
		"Moonbeam Local Testnet",
		"moonbeam_local_testnet",
		ChainType::Local,
		moonbeam_local_testnet_genesis,
		vec![],
		None,
		Some(DEFAULT_PROTOCOL_ID),
		None,
